...
function logData() {
    gutil.log(
        gutil.colors.bold(
            gutil.colors.green.apply(this, arguments)
        )
    );
}
...
gulp.task('build:index', function() {
    ...
    var html = html.replace(new RegExp('{dir}', 'g'), 'ltr');
    //var rtl = html.replace(new RegExp('{dir}', 'g'), 'rtl');

    return file('index.html', html, { src: true }).pipe(gulp.dest('public'));
});
...
if(production) {
    ...
    gulp.task('default', function(callback) {
        runSequence('build:css', 'build:essentials', 'minifycss', 'bless', 'base64-css', 'uglify', 'build:index', function() {
            callback();
            gutil.log(
    ...
    gulp.task('default', function(callback) {
        ...
        runSequence('react:development', 'build:css', 'build:essentials', 'base64-css', 'build:index', ['watch'], callback);
    });
}

gulp.task('development', function(callback) {
    //runSequence('react:development', 'react:app', 'notify', 'express:watch', callback);
    runSequence('react:development', 'react:app', callback);
});
...
gulp.task('watch', function() {
    ...
    gulp.watch(paths.index, ['build:index'], ready);
    gulp.watch(paths.l20n, ['react-l20n:watch'], ready);
    gulp.watch(paths.scss, ['rebuild:css'], ready);
    gulp.watch(paths.ttf, ['base64-css:watch'], ready)
    gulp.watch(paths.jsx.concat(paths.scss), ['development'], ready);

    gutil.log(
        gutil.colors.bgGreen(
            gutil.colors.white(
                gutil.colors.bold('[          WATCHING!!!          ]')
            )
        )
    );
});
